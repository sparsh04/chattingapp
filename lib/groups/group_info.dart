import 'package:chattingapp/groups/add_members.dart';
  ...
  Future onLeaveGroup() async {
    if (!checkAdmin()) {
      ...
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(builder: (_) => TabsScreen(0)),
        (route) => false,
      );
    } else if (checkAdmin() && membersList.length == 1) {
      setState(() {
        isLoading = true;
      });
      membersList = [];
      ...
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(builder: (_) => TabsScreen(1)),
        (route) => false,
      );
    }
  }
